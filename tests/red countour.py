...
def detect_purple_objects(image):
    img_hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    lower_purple = np.array([0,50,50])
    upper_purple = np.array([10,255,255])
    mask0 = cv2.inRange(img_hsv, lower_purple, upper_purple)

    lower_purple = np.array([170,50,50])
    upper_purple = np.array([180,255,255])
    mask1 = cv2.inRange(img_hsv, lower_purple, upper_purple)
...
if __name__ == '__main__':
    while True:
        ...
        detect_purple_objects(auv.get_image_front())
